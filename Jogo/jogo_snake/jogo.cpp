...
int comidaX, comidaY;
short sDirection = RIGHT; ///Direcao atual da cobra
extern bool gameOver;
extern int pontos;

int cobraTam = 5;
int posX[60]={20,20,20,20,20}, posY[60]={20,19,18,17,16}; ///Posicao da Cobra inicia no meio, inicia as primeiras 5 posicoes que � o tamanho inicial da cobra
...
void unidade(int x, int y)
{
    ...
    glBegin(GL_LINE_LOOP); ///No caso � o loop de linhas

        /// Vertice 2D (x,y)
        /// Posicoes vao mudando at� que se forme o quadrado
        glVertex2f(x,y);
        glVertex2f(x+1,y);
        glVertex2f(x+1,y+1);
        glVertex2f(x,y+1);
...
void desenhaCobra()
{
    ...
    ///verifica a direcao que esta indo e adiciona (remove) posicoes
    if(sDirection==UP){
        posY[0]++;
    ...
    for(int i=0; i<cobraTam; i++){
        ...
        glRectd(posX[i], posY[i], posX[i]+1, posY[i]+1); ///gera um quadrado das posicoes
    }
    ...
    if(posX[0]==comidaX && posY[0]==comidaY){ ///se a cabeca da cobra e a comida estiverem na mesma posicao
        pontos++;
        cobraTam++;
        if(cobraTam>MAX){
...
/// funcao para gerar a posicao da comida
