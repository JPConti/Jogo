...
#define COLUNAS 40
#define LINHAS 40
#define FPS 10 /// Tempo de geracao de quadros, por segundo
...
/// FUNCAO QUE INICIA VALORES ANTES DA EXECUCAO DO PROGRAMA
void init()
{
...
int main(int argc, char **argv){
    ...
    glutInit(&argc, argv);
    glutInitDisplayMode(GLUT_RGB | GLUT_DOUBLE); ///Modo de exibicao RGB com Dupla janela (dois buffers em exibicao)
...
void display_callback()
{
    ...
    if(gameOver){
        ...
        char text[50] = "Sua pontuacao foi de: ";
...
void reshape_callback(int w, int h)
{
    ...
    glMatrixMode(GL_PROJECTION); ///Matriz de progecao
    glLoadIdentity(); ///Garante que nao havera alteracoes na matriz, sera padrao inicial

    ///Projecao Ortografica -> em relacao a janela (esq, dir, bai, cima, perto, longe (dos objetos mostrados, como eixo z))
...
void timer_callback(int)
{
    glutPostRedisplay(); ///funcao de exibicao sera chamada (display_callback) para a criacao de um novo quadro
    glutTimerFunc(800/FPS, timer_callback, 0);
}

/// Funcao que le o comando  digitado no teclado
void keyboard_callback(int key, int,int) ///le apenas o comando, nao precisa das posicoes
{

    switch(key)
    {
    case GLUT_KEY_UP:
        ///Primeiro verifica se nao esta indo na direcao oposta
        ///pq a cobra nao vira 180 graus
        if(sDirection!=DOWN){
            sDirection=UP;
        }
        break;
